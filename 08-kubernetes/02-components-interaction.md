...
- The **scheduler** continuously monitors etcd for unscheduled pods
...
- API server authenticates and updates etcd with the unscheduled Pod.
- Scheduler detects the unscheduled Pod, selects a node, and updates the Pod spec with nodeName.
- Pod controller (in the controller-manager) detects the updated Pod spec and marks it as scheduled.
- Kubelet on the selected node pulls the image and starts the container.
